...
int init_encoder (const char* output_file, int sourceWidth, int sourceHeight) {
    ...
    sws_context = sws_getContext (SOURCE_WIDTH, SOURCE_HEIGHT, AV_PIX_FMT_RGB24, WIDTH, HEIGHT, AV_PIX_FMT_YUV420P,
...
int write_video_frame (const uint8_t* rgb_data) {
    av_image_fill_arrays (rgb_frame->data, rgb_frame->linesize, rgb_data, AV_PIX_FMT_RGB24, SOURCE_WIDTH, SOURCE_HEIGHT,
                          1);

    sws_scale (sws_context, rgb_frame->data, rgb_frame->linesize, 0, SOURCE_HEIGHT, video_frame->data,
