...
std::optional<ApplicationContext::PlaylistDefinition> ApplicationContext::buildPlaylistDefinition (
    const JSON& playlistJson, const std::string& fallbackName) const {
...
std::vector<std::filesystem::path> ApplicationContext::collectPlaylistItems (const JSON& playlistJson,
...
void ApplicationContext::loadSettingsFromArgv () {
    ...
    backgroundMode.add_argument ("-r", "--screen-root")
        ...
        .action ([this, &lastScreen] (const std::string& value) -> void {
            ...
            this->settings.general.uvOffset [lastScreen] = this->settings.render.window.uvOffset;
        })
        .append ();
    ...
    backgroundGroup.add_argument ("--playlist")
        .help (
            "Uses a playlist from wallpaper engine's config.json. If used after --screen-root it is applied to that screen, otherwise it is used in window mode.")
        .action ([this, &lastScreen] (const std::string& value) -> void {
    ...
    backgroundGroup.add_argument ("--scaling")
        .help (
            "Scaling mode to use when rendering the background, this applies to the previous --window or --screen-root output, or the default background if no other background is specified")
    ...
    backgroundGroup.add_argument ("--offset-x")
        .help (
            "UV X offset applied when rendering the background, this applies to the previous --window or --screen-root output, or the default background if no other background is specified ")
        .default_value (0)
        .action ([this, &lastScreen] (const std::string& value) -> void {
            const float offset = std::stof (value);

            if (this->settings.render.mode == DESKTOP_BACKGROUND) {
                this->settings.general.uvOffset [lastScreen].x = offset;
            } else {
    ...
    backgroundGroup.add_argument ("--offset-y")
        .help (
            "UV Y offset applied when rendering the background, this applies to the previous --window or --screen-root output, or the default background if no other background is specified ")
        .default_value (0)
        .action ([this, &lastScreen] (const std::string& value) -> void {
            const float offset = std::stof (value);

            if (this->settings.render.mode == DESKTOP_BACKGROUND) {
                this->settings.general.uvOffset [lastScreen].y = offset;
            } else {
    ...
    backgroundGroup.add_argument ("--clamp")
        .help (
            "Clamp mode to use when rendering the background, this applies to the previous --window or --screen-root output, or the default background if no other background is specified")
    ...
    try {
        ...
        throw std::runtime_error (std::string (e.what ()) + ". Use " + std::string (this->m_argv [0]) +
