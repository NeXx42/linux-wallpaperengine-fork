...
WallpaperUniquePtr WallpaperParser::parse (const JSON& file, Project& project) {
    switch (project.type) {
        case Project::Type_Scene: return parseScene (file, project);
        case Project::Type_Video: return parseVideo (file, project);
        case Project::Type_Web: return parseWeb (file, project);
        default: sLog.exception ("Unexpected project type value found... This is likely a bug");
    }
}
...
SceneUniquePtr WallpaperParser::parseScene (const JSON& file, Project& project) {
    ...
    return std::make_unique<Scene> (
        WallpaperData {.filename = "", .project = project},
        SceneData {
            .colors =
                {
            ...
            .camera = {.fade = general.optional ("camerafade", false),
                       ...
                       .projection = {.width = projection.require<int> ("width", "Projection must have a width"),
